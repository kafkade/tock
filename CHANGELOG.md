...
- **Scheduled tasks / calendar-slot scheduling** (#154): tasks now have a first-class `scheduled_for` slot — the GTD distinction between a *deadline* (when a task is due) and when you actually plan to work on it — with optional time-of-day. Set it inline with a `sched:`/`scheduled:` sigil in `tock add` and `tock modify` (e.g. `sched:2026-06-01` or `sched:2026-06-01T14:30`), or with the dedicated `tock schedule <sid> <when>` command, which accepts natural-language day-and-time phrases (`tomorrow 9am`, `next friday 15:30`, `2026-06-01 14:00`); `tock unschedule <sid>` clears the slot. A new `scheduled` view (`tock scheduled` / `tock view scheduled`) lists every open task that has a planned slot, and `tock agenda [day]` shows a day's agenda — scheduled tasks (timed first in chronological order, then all-day) interleaved with that day's time blocks, flagging any task whose start time falls inside a time block as an overlap. Tasks scheduled for today (or earlier) now also surface in the Today view and under the `+TODAY` filter, and you can filter on the slot directly with `+SCHEDULED` and `scheduled:<date>`. The slot shows in `tock show`, in compact list rows (`sched:…`), and in list/detail JSON, and round-trips through JSON import/export and the Apple (UniFFI) bindings
- **Checklist items** (sub-task checkboxes, #153): tasks can now carry an ordered list of lightweight checklist items — a title plus a done state, nothing else (no scheduling, tags, or nesting), per architecture §2.1. A new `tock checklist` command (alias `cl`) manages them by 1-based position: `add <sid> <text…>` appends an item, `ls <sid>` lists them with `[x]`/`[ ]` markers and a `done/total` header, `check`/`uncheck <sid> <index>` toggle completion, `rm <sid> <index>` removes and renumbers, and `reorder <sid> <from> <to>` moves an item. Checklist progress (e.g. `1/3`) surfaces in `tock list`/compact rows, in `tock show`, and in the TUI detail pane. Items round-trip through JSON export/import via a `checklist` array (the target shape for the future Things 3 checklist mapping) and are covered by `tock undo`/`redo`
- **Task annotations** (`annotate` command + storage, #151): tasks can now carry append-only, timestamped annotations — short notes that accumulate over time, separate from a task's single free-form `notes` field. `tock annotate <sid> "<text>"` attaches a note; `tock denotate <sid> <index>` removes one by the 1-based index shown in `tock show`. `tock show` lists annotations (numbered, with dates) in both table and `--json` output. Annotations round-trip through JSON export/import (`annotations: [{ body, created_at }]`) and render nested under each task in the Markdown `task-list` export. The Taskwarrior importer now maps `annotations` to real annotations — preserving each note's `entry` timestamp — instead of flattening them into `notes`. Annotations are covered by the existing sync and undo machinery
- **User configuration file** (`~/.config/tock/config.toml`, #149): tock now reads a typed TOML config on startup, so preferences persist across invocations instead of living only in flags. The file layers under env vars and CLI flags (precedence: defaults → file → env → flag) and every section is optional. `[general] default_view` sets the view shown by a bare `tock`; `[urgency]` overrides the urgency coefficients (`deadline`, `start_date`, `priority`, `age`, `tag`, `project`, `next`, `blocked`, `waiting`) plus per-tag boosts via `[urgency.tags]`, and the weights flow through the storage layer so the cached urgency always reflects your config; `[focus]` supplies Pomodoro defaults (work/short-break/long-break/cycles) that CLI flags still override; `[uda.<key>]` declares user-defined attributes that merge over the per-vault DB (config wins) and surface in `tock uda list`; `[keys]` remaps the TUI keybindings; and `[notifications] enabled = false` silences desktop/terminal notifications globally. A new `tock config` command inspects and scaffolds it: `config show` (effective merged config as TOML, or `--json`), `config path` (the resolved file location), and `config init` (writes a documented sample, `--force` to overwrite). The location honors `--config <path>`/`TOCK_CONFIG` and `XDG_CONFIG_HOME`. See the new [configuration guide](docs/configuration.md)
...
- **Undo / redo** (#150): `tock undo` reverses the last mutating command and `tock redo` re-applies it, with clear feedback (`Undid: done #42` / `Redid: done #42`, or `Nothing to undo` / `Nothing to redo`). It covers task, tag, dependency, project/area/heading, annotation, time-tracking, focus, habit, UDA, saved-report, and context mutations, and cascade effects revert together as one step — completing a task that spawns the next recurrence, auto-stops a linked focus session, and closes an open time block all undo in a single `tock undo`. The undo history is persisted across invocations, linear (starting a new action clears the redo stack), depth-bounded, and kept device-local — it is never synced
- **Browser self-service portal + admin console polish** (#131): the self-hosted web console gains the account-management pieces that previously required the CLI. A signed-in user gets a **Your account** portal to manage their own **devices** and **sessions** (list, revoke a single one, or "sign out all other sessions" — the current session is flagged), re-display their add-device **Setup Code** (with QR + print) without a server round-trip, and perform a full **password rotation** end-to-end in the browser: the new password re-derives the Unlock Root Key, re-wraps the Vault Key, and mints a fresh SRP verifier entirely in WASM, uploading only the non-secret verifier material and re-wrapped header (the server stays zero-knowledge; the Secret Key and Emergency Kit are unchanged). The **first-run wizard** now sets the registration policy and public server address (defaulting to the current origin) right after admin creation, and the **admin console** adds an editable public-address setting plus a usage/health panel (accounts by role/status, vault/device/event counts, encrypted storage size, and a live `/health` check). New self-scoped server endpoints back the portal (`PUT /v1/account/srp-verifier`, `GET/DELETE /v1/account/sessions…`, `GET/DELETE /v1/account/devices…`), a new `GET /v1/admin/stats` feeds the panel, `GET/PUT /v1/admin/settings` and `GET /v1/server/info` carry the public address, and account registration now uploads the wrapped vault header so browser-only accounts can rotate and new devices can recover their keys
...
- **Things 3 import** (#152): `tock import things3 -f <file>` imports a Things 3 canonical JSON export, recreating the full hierarchy — Areas → areas, Projects → projects (area inherited), Headings → project headings, and To-Dos → tasks. Field mapping follows the Things model: `when` (today/evening/`<date>`) maps to the task start date (with the evening flag), `someday` moves the task to the Someday view with a `+someday` tag, `deadline` maps to the due date, and tags (with `/` paths preserved), notes, open/completed/canceled status, and repeat rules carry over. Checklist items degrade gracefully into the task's notes as `- [ ]`/`- [x]` lines until a native checklist domain lands. Trashed items are skipped unless you pass `--include-trash`, and the importer prints a summary of everything created, degraded, skipped, and any fields it couldn't map
